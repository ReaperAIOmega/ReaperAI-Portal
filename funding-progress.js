...
ready.then(async({supabase})=>{
  const [readiness,applications,matches]=await Promise.all([
    ...
    supabase.from('funding_matches').select('id,lender_name,program_name,product_type,eligibility,rank,estimated_amount,reasons,created_at').order('rank',{ascending:true}),
  ]);
  ...
  render(document.getElementById('match-list'),[{label:'Lender',key:'lender_name'},{label:'Program',key:'program_name'},{label:'Product',key:'product_type'},{label:'Eligibility',key:'eligibility'},{label:'Rank',key:'rank'},{label:'Rationale',render:r=>Array.isArray(r.reasons)?r.reasons.join('; '):''}],matches.data||[]);
